...
const NoteCard = memo(({ note, added, onAddToCart }: { note: Note; added: boolean; onAddToCart: (note: Note) => void }) => (
  <div
    className="glass-card-v2 bg-card/40 border border-white/5 rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300 flex flex-col justify-between group h-full"
  >
    <Link to={`/notes/${note.id}`} className="relative aspect-[4/3] overflow-hidden bg-void/50 block border-b border-white/5">
      <img
        src={note.thumbnail_url}
        alt={note.title}
        className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
      />
      <div className="absolute top-3 left-3 bg-void/90 text-primary border border-primary/20 text-[9px] font-display font-semibold px-2.5 py-1 rounded-full uppercase tracking-wider shadow-md">
        {(note as any).course?.name || 'Medical'}
      </div>
      <div className="absolute bottom-3 right-3 bg-primary text-void text-xs font-display font-extrabold px-3 py-1 rounded-xl shadow-md">
        ₹{note.price}
      </div>
    </Link>
    <div className="p-5 flex flex-col flex-1 justify-between gap-4">
      <div>
        <span className="text-[10px] font-sans font-semibold text-primary tracking-wider uppercase">
          {note.subject}
        </span>
        <Link
          to={`/notes/${note.id}`}
          className="font-display font-bold text-base text-white hover:text-primary transition-colors mt-1 block line-clamp-2"
        >
          {note.title}
        </Link>
      ...
      <div className="flex gap-2 mt-2">
        {added ? (
          <Link
            to="/cart"
            className="w-full py-2.5 bg-primary/10 text-primary border border-primary/20 font-display text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 hover:bg-primary/20 transition-all"
          >
          ...
          <button
            ...
            className="w-full py-2.5 bg-primary text-void font-display text-xs font-bold rounded-xl flex items-center justify-center gap-1.5 hover:bg-primary-dark transition-all"
          >
    ...
    <div className="bg-void min-h-screen text-white pt-24 pb-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/5 pb-8">
          ...
          <div className="relative w-full md:max-w-md">
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-card border border-white/10 focus:border-primary focus:ring-1 focus:ring-primary/50 outline-none rounded-xl text-sm transition-all text-white"
            />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <aside className="hidden lg:block lg:col-span-3 bg-card/45 backdrop-blur-lg border border-white/10 rounded-2xl p-6 shadow-2xl sticky top-24">
            <div className="flex items-center gap-2 font-display font-bold text-white mb-6 border-b border-white/5 pb-3">
        ...
        {mobileFilterOpen && (
          <div className="fixed inset-0 z-50 lg:hidden flex">
            ...
            {/* Drawer body */}
            <div className="relative ml-0 mr-auto w-full max-w-xs h-full bg-card border-r border-white/10 shadow-2xl p-6 flex flex-col justify-between z-10 animate-slide-right text-white">
              <div>
